...
sub AnyEvent::common_sense {
   local $^W;
   ${^WARNING_BITS} ^= ${^WARNING_BITS} ^ "\x0c\x3f\x33\x00\x0f\xf0\x0f\xc0\xf0\xfc\x33\x00\x00\x00\x00";
   $^H |= 0x7c0;
}
# generated for perl 5.018004 built for darwin-thread-multi-2level
package AnyEvent;
sub CYGWIN(){0}
sub WIN32(){0}
sub F_SETFD(){2}
...
sub MSG_DONTWAIT(){undef}
sub MSG_FASTOPEN(){undef}
sub MSG_MORE(){undef}
sub MSG_NOSIGNAL(){undef}
sub TCP_CONGESTION(){undef}
sub TCP_CONNECTIONTIMEOUT(){32}
sub TCP_CORK(){undef}
sub TCP_DEFER_ACCEPT(){undef}
sub TCP_FASTOPEN(){undef}
sub TCP_INFO(){undef}
